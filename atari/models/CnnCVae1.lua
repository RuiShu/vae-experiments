local CnnCVae1 = torch.class("CnnCVae1")
local c = require 'trepl.colorize'
require 'nngraph'
require 'threads'
require 'nnutils'

function CnnCVae1:__init(cmd, network)
  -- build network
  self.cmd = cmd
  if cmd.gpu > 1 then
  ...
  self.rec = nn.GaussianCriterion(3,true)
  self.mse = nn.MSECriterion(false)
end

function CnnCVae1:getNetwork()
...
function CnnCVae1:parallelBuild(network)
...
function CnnCVae1:build()
  ...
  deconv:add(nn.SpatialFullConvolution(64,6, 8,8, 2,2, 1,0))
  deconv:add(nn.SplitTensor(1,3,3))
  ...
  local predMuLv = deconv(hxac)
  -- combine
  local network = nn.gModule({x, a, y}, {pmuLv, muLv, predMuLv})
  return network
end

function CnnCVae1:feval(x, minibatch)
  ...
  local pmulv, mulv, predMuLv = unpack(self.network:forward({input, action, target}))
  local kldErr = self.kld:forward(mulv, pmulv)
  local recErr = self.rec:forward(predMuLv, target)
  local predErr = self.mse:forward(predMuLv[1], target)/batchSize
  local nElbo = kldErr + recErr
  local dmulv, dpmulv = self.kld:backward(mulv, pmulv)
  local dpredMuLv = self.rec:backward(predMuLv, target)
  local gradOut = {dpmulv, dmulv, dpredMuLv}
  self.network:backward({input, action, target}, gradOut)
  -- record
  self.recordName = {'nElbo', 'KLD', 'REC', 'MSE'}
  self.record = {nElbo=nElbo, KLD=kldErr, REC=recErr, MSE=predErr}
  return nElbo, self.gradients
end

function CnnCVae1:sendRecord()
  local comm = {}
...
function CnnCVae1:getParameters()
